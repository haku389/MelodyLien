# StreetMelody API — Cloud Run デプロイ手順
...
# 任意で上書き:
# PROJECT_ID=xxx REGION=asia-northeast1 SERVICE=streetmelody-api bash scripts/deploy-cloudrun.sh
```
...
`apps/ios/StreetMelody/Services/APIService.swift` の本番 baseURL（`#else` 側、現在 `https://api.streetmelody.app/api`）を、

- **A) Cloud Run の URL** にそのまま変更する（`https://<service-url>/api`）、または
- **B) カスタムドメイン** `api.streetmelody.app` を Cloud Run にマッピングし、現状のままにする
  （`gcloud run domain-mappings create --service streetmelody-api --domain api.streetmelody.app …` ＋ DNS 設定）
