...
- **認証＋per-user 接続プラン** `docs/auth-peruser-plan.md`: RLS のため per-user 書き込みには JWT か service_role が必須で、**方式A（クライアント直結・Supabase直叩き＝web と統一・推奨）** と **方式B（Go API 集約＋service_role）** に分岐。方式確定後にフェーズ実装（iOS 匿名Auth→per-user 読み書き→ローダー再接続）。**この分岐はユーザー判断待ち**。
