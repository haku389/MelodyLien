...
- **マージ**: `feat/supabase-catalog-thumbnail` を `main` へ fast-forward マージ（ローカル完了）。**push はこの作業環境に GitHub 認証情報が無く失敗**（`could not read Username`）。ユーザーの認証済み端末から `git push origin main` が必要。remote=`github.com/haku389/MelodyLien.git`。
- **Cloud Run デプロイ成果物**（gcloud/docker 未インストールのため実デプロイは未実行・ユーザー実行用に準備）:
  ...
  - 手順書 `docs/deploy-cloudrun.md`。デプロイ後 iOS の本番 baseURL を Cloud Run URL に差し替え or `api.melodylien.app` をドメインマッピング。
